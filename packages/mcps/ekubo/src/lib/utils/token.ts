...
import { tokenAddresses } from '../contracts/addresses.js';

import { Uint256, Call, Account } from 'starknet';
import { NEW_ERC20_ABI, OLD_ERC20_ABI } from '../contracts/erc20.js';
...
export async function detectAbiType(address: string, provider: Provider) {
  try {
    ...
    console.warn(
      `Couldn't detect ABI type, defaulting to NEW_ERC20_ABI: ${error.message}`
    );
    return NEW_ERC20_ABI;
  }
}
