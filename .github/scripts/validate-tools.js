...
async function validateAllTools() {
  ...
  for (const [mcpName, config] of Object.entries(mcpsConfig)) {
    ...
    try {
      ...
      const missingTools = declaredTools.filter(tool => !actualTools.includes(tool));
      if (missingTools.length > 0) {
        console.error(`❌ Tools declared in config but not implemented: ${missingTools.join(', ')}`);
        hasErrors = true;
      }
      
      // Check for undeclared tools
      const undeclaredTools = actualTools.filter(tool => !declaredTools.includes(tool));
      if (undeclaredTools.length > 0) {
        console.error(`❌ Tools implemented but not declared in config: ${undeclaredTools.join(', ')}`);
        hasErrors = true;
      }
